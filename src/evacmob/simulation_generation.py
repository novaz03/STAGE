...
_BEHAVIOR_COMPACT = 0
_BEHAVIOR_INTERMEDIATE = 1
_BEHAVIOR_EXTENSIVE = 2
...
def generate_random_trajectories(
    ...
    *,
    ...
    behavior_ids = (
        [_BEHAVIOR_COMPACT] * (base + (1 if rem > 0 else 0))
        + [_BEHAVIOR_INTERMEDIATE] * (base + (1 if rem > 1 else 0))
        + [_BEHAVIOR_EXTENSIVE] * base
    )
    behavior_ids = np.asarray(behavior_ids, dtype=int)
    rng.shuffle(behavior_ids)

    def simulate_one_person(home_idx: int, behavior_id: int) -> list[tuple[int, int]]:
        ...
        for day in range(1, _SIMULATION_DAYS + 1):
            ...
            if behavior_id == _BEHAVIOR_COMPACT:
            ...
            elif behavior_id == _BEHAVIOR_INTERMEDIATE:
                if day == 1:
            ...
            elif behavior_id == _BEHAVIOR_EXTENSIVE:
                if day == 1:
                    gas = pick_strict(
                        ref_idx=int(home_idx),
    ...
    for row in people_df.itertuples(index=False):
        behavior_id = int(behavior_ids[int(row.person_id)])
